...
public class PeonMoveController {
  ...
  private OnceXTimes<Peon> findPathClosure = new OnceXTimes<Peon>(90, true, new Closure<Peon>() {
    ...
    public void run(Peon peon) {
      ...
      // Target is not available and we want to stand in place, so we generate path which leads nowhere.
      if (destination == null && peon.getCurrentPath() != null && peon.getCurrentPath().fastGetPath() != null) {
        Path current = peon.getCurrentPath().fastGetPath();
        if (current.getPoints().size() > 0) { // If size == 0 then this path was generated by exactly the lines below.
          Path p = new Path(current.getStart(), new LinkedList<Joint>());
