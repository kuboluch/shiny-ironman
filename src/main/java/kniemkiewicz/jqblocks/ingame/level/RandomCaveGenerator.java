...
public class RandomCaveGenerator {
  ...
  ArrayList<Vector2i> getOrderedPoints(int dx, int dy, int maxPointsPerWall, Random random) {
    int points = random.nextInt(maxPointsPerWall) + 1;
  ...
  void makeCave(float cx, float cy, float dx, float dy, int maxPointsPerWall, Random random) {
    assert maxPointsPerWall >= 1;
    ...
    generateContour(maxPointsPerWall, random, width, height, dd, x0, y0);
  ...
  private void generateContour(int maxPointsPerWall, Random random, int width, int height, int dd, int x0, int y0) {
    int x1 = x0 + dd,         y1 = y0 + dd;
    int x2 = x0 + width - dd, y2 = y0 + height - dd;
    List<List<Vector2i>> allPoints = new ArrayList<List<Vector2i>>();
    {
      ...
      List<Vector2i> points = getOrderedPoints(width - 2 * dd, dd, maxPointsPerWall, random);
    ...
    }
    {
      ...
      List<Vector2i> points = getOrderedPoints(height - 2 * dd, dd, maxPointsPerWall, random);
      for (Vector2i v : points) {
        v.swap();
        v.add(corner);
      }
    ...
    }
    {
      ...
      ArrayList<Vector2i> points = getOrderedPoints(width - 2 * dd, dd, maxPointsPerWall, random);
      for (Vector2i v : points) {
        v.add(corner);
      }
